...
public class PaymentService {
	...
	public void findCreditCard(CreditCard creditCard) throws CreditCardNotFoundException,InvalidCardDetailsException{
		CreditCardEntity cce = (CreditCardEntity) creditCardRepository.findById(creditCard.getCardNumber()).orElse(null);
		if(cce==null){
	...
	public void confirmBooking(HttpSession session) throws Exception{
		...
		UserEntity ue = userRepository.findById(booking.getName()).orElse(null);
		
		FlightEntity flightEntity = flightRepository.findById(booking.getFlightId()).orElse(null);
		...
		for (Passenger passenger : passengerList) {
			...
			TicketEntity t = ticketRepository.findById(booking.getPnr().toString()).orElse(null);
			pe.setTicket(t);
